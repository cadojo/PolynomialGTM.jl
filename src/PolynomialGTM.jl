...
"""
@memoize function GTM(; stm=false, name=:GTM)
    ...
    return ODESystem(eqs, t, x, p; defaults=defaults, name=Symbol(modelname))

end

"""
Returns a `DifferentialEquations`-compatible `ODEFunction` for GTM dynamics.
The `stm`, and `name` keyword arguments are passed to `GTM`. All other
keyword arguments are passed directly to `ODEFunction`.
...
```
"""
@memoize function GTMFunction(; stm=false, name=:GTM, kwargs...)
    defaults = (; jac=true)
